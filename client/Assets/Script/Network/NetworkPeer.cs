...
namespace Network
{
    ...
    public class NetworkPeer
    {
        ...
        public NetworkPeer(string name, MessageMeta meta )
        {
            _name = name;
            _meta = meta;
        }
        ...
        void LogMessage( uint msgid, object msg)
        {
            switch (msgid)
            {
                case (UInt32)NetworkEvent.Connected:
                    Debug.Log(string.Format("[{0}] Connected", _name));
                    break;
                case (UInt32)NetworkEvent.Closed:
                    Debug.Log(string.Format("[{0}] Closed", _name));
                    break;
                default:                    
                    Debug.Log(string.Format("[{0}] {1}|{2}", _name, msg.GetType().FullName, _printer.Print(msg )));
                    break;
            }
