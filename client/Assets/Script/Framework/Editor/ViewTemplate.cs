...
namespace Framework
{
    class ViewTemplate
    {
        ...
        public static string FileName(DataContext ctx)
        {
            return ClassName(ctx) + ".codegen.cs";
        }
        ...
        {            
            ...
            foreach (DataContext widgetContext in widgetContextList)
            {
                FindWidgetlAssignToVar(gen, viewContext, widgetContext);
            }
        ...
        public static void FindWidgetlAssignToVar(CodeGenerator gen, DataContext viewContext, DataContext widgetContext)
        {
            var path = ObjectUtility.GetGameObjectPath(widgetContext.gameObject, viewContext.gameObject);

            gen.PrintLine(WidgetVariable(widgetContext), " = trans.Find(\"", path, "\").GetComponent<", widgetContext.Type.ToString(), ">();");
        }
        ...
        public static void WriteFile( CodeGenerator gen, DataContext viewContext )
        {
            CodeUtility.WriteFile( string.Format("Assets/Script/View/{0}", FileName(viewContext) ), gen.ToString() );
        }
